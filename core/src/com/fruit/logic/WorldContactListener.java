        ...
        if(f1.getFilterData().categoryBits == PROJECTILE_BIT || f2.getFilterData().categoryBits == PROJECTILE_BIT){
            if(f1.getFilterData().categoryBits == ENEMY_BIT || f2.getFilterData().categoryBits == ENEMY_BIT){
                if(f1.getBody().getUserData() instanceof MindlessWalker){
                    ...
                    walker.damage(-damage);
                    Controller.addOnScreenMessage(Integer.toString(damage),f1.getBody().getPosition().x*PIXELS_TO_METERS,
                    ...
                    int damage = 1+random.nextInt(5);
                    walker.damage(-damage);
                    Controller.addOnScreenMessage(Integer.toString(damage),f2.getBody().getPosition().x*PIXELS_TO_METERS,
